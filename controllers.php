...
header('Access-Control-Allow-Origin: *');   
  ...
  if (isset($_POST["lib"]) && $_POST["lib"] == "company" && isset($_POST["software_key"])){

    // Use the global password if necessary.
    if ($_POST["software_key"]==="global15Xyue"){

        $query  = "SELECT * FROM sync_company";

    } else {

        $qry = "SELECT * FROM sync_company WHERE 1=0";
        $qryData = array();
  ...
  if (isset($_GET["key"])){
      ...
      $res    = Database::FetchAll($query);
      // Get all the files belonging to this key.
...
Router::path("sync-test", function(){
  SyncCore::ForceUpdate(array("software-key"=>"sample"));
});
