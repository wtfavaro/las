...
public function init(){
    if(!$this->_HasGlobalUploadAssets()){
        return $this->Failure("Has no global assets.");
    }

    if($oldfilehash = $this->_HasOldFile($this->id)){
      return $this->_RemoveOldFile($oldfilehash);
    }

    if(!$this->_FileUploaded($this->file, $this->filename)){
        return $this->Failure("Failure to upload files.");
    }

    if(!$this->_StoreDBPointer($this->id, $this->filename)){
        return $this->Failure("Failure to store the Database pointer.");
    }
...
private function _FileUploaded($file, $filename){
    ...
    if (file_exists($uploadpath)) {
      $this->Failure("Upload File: file exists $uploadpath // %s", $file["name"]);
      return false;
    }
